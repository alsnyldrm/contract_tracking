...
async function saveLdap() {
  ...
  alert('LDAPS ayarları kaydedildi');
}

async function testLdap() {
  const data = await api('/api/settings/ldap/test', { method: 'POST', body: JSON.stringify({}) });
  document.getElementById('ldapTestResult').textContent = data.message;
}
...
async function saveSaml() {
  ...
  alert('SAML ayarları kaydedildi');
}
...
async function testSmtp() {
  ...
  document.getElementById('smtpResult').textContent = result.message;
}
